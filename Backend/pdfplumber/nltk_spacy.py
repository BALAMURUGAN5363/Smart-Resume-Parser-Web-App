...
output_txt_path = 'H:/OCR/output.txt'
output_json_path = 'output.json'

nlp = spacy.load("en_core_web_sm")

# Read extracted text from external resource
...
for page in pages[1:]:
    ...
    for ent in doc.ents:
        if ent.label_ not in page_data["entities"]:
            ...
            page_data["entities"][ent.label_] = [page_data["entities"][ent.label_], ent.text]
    
    invoice_data[f"page_{page_number}"] = page_data

# Save extracted structured data as JSON
